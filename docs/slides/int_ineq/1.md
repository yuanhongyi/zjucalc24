...
$$\int_{a}^{b}xf(x)\mathrm{d}x\geq\frac{1}{2}[b\int_{0}^{b}f(x)\mathrm{d}x - a\int_{0}^{a}f(x)\mathrm{d}x]$$
...
$$f(x)=\int_{0}^{x}(t - t^{2})\sin^{2n}t\mathrm{d}t\leq\frac{1}{(2n + 2)(2n + 3)}$$
...
!!! Answer
    **证法一**
    ...
    故\(F(x)\)为\([a,b]\)上的减函数。$F(b)\ge F(a)=0$
