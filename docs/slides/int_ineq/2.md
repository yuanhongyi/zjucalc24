...
!!! Answer
    ...
    $\iff 1\le\dfrac{F(1)}{G(1)} = \dfrac{F(1)-F(0)}{G(1)-G(0)} = \dfrac{F'(\xi)}{G'(\xi)} = \dfrac{2\int_0^{\xi}f(t)\mathrm{d}t}{f^2(\xi)}$
...
!!! Note
    ...
    - 取$f(x) = \ln x$，区间为$[0,1]$，得到$\int_0^1\ln f(x)\mathrm{d}x\le \ln \left[\int_0^1 f(x)\mathrm{d}x\right]$
